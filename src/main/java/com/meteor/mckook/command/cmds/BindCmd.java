...
public class BindCmd extends SubCmd {
    ...
    private void handleAddBinding(CommandSender sender, String playerName, String kookId) {
        // TODO: 实现添加绑定的逻辑
        // 例如: plugin.getBindingManager().addBinding(playerName, kookId);
        //获取平台实例并异步调度任务
        //在线程里通过玩家名字查 UUID
        //解析 kook_id 号
        //更新绑定映射
        LinkService currentLinkService = getLinkService();
        currentLinkService.linkRepository.bind(playerName,kookId);
        sender.sendMessage(ChatColor.GREEN + String.format("尝试为玩家 %s 和 Kook ID %s 添加绑定...", playerName, kookId));
        sender.sendMessage(ChatColor.YELLOW + "(提示: 实际绑定逻辑需在此处实现)");
    }

    private void handleGetKookIdByPlayer(CommandSender sender, String playerName) {
        // TODO: 实现通过玩家名获取 Kook ID 的逻辑
        LinkService currentLinkService = getLinkService();
        currentLinkService.linkRepository.bindgetKookIdByPlayerName(playerName);
        sender.sendMessage(ChatColor.GREEN + String.format("尝试获取玩家 %s 绑定的 Kook ID...", playerName));
        sender.sendMessage(ChatColor.YELLOW + "(提示: 实际查询逻辑需在此处实现)");
    }

    private void handleGetPlayerByKookId(CommandSender sender, String kookId) {
        // TODO: 实现通过 Kook ID 获取玩家名的逻辑

        LinkService currentLinkService = getLinkService();
        currentLinkService.linkRepository.bindgetPlayerNameByKookId(kookId);
        sender.sendMessage(ChatColor.GREEN + String.format("尝试获取 Kook ID %s 绑定的玩家名...", kookId));
        sender.sendMessage(ChatColor.YELLOW + "(提示: 实际查询逻辑需在此处实现)");
    }
    ...
    private void handleRemoveBindingByPlayer(CommandSender sender, String playerName) {
        // TODO: 实现通过玩家名移除绑定的逻辑
        LinkService currentLinkService = getLinkService();
        currentLinkService.linkRepository.bindremoveByplayerName(playerName);
        sender.sendMessage(ChatColor.GREEN + String.format("尝试移除玩家 %s 的绑定...", playerName));
        sender.sendMessage(ChatColor.YELLOW + "(提示: 实际删除逻辑需在此处实现)");
    }

    private void handleRemoveBindingByKookId(CommandSender sender, String kookId) {
        // TODO: 实现通过 Kook ID 移除绑定的逻辑
        LinkService currentLinkService = getLinkService();
        currentLinkService.linkRepository.bindremoveByKookId(kookId);
        sender.sendMessage(ChatColor.GREEN + String.format("尝试移除 Kook ID %s 的绑定...", kookId));
        sender.sendMessage(ChatColor.YELLOW + "(提示: 实际删除逻辑需在此处实现)");
    }
